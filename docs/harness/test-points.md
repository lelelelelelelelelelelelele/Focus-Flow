...
| TP5 | 验证环境口径（sandbox / CI 测试在哪算数） | Episode#1 D1/D2 | `covered-now` | CI 加 `test` job（push/PR 跑 vitest run），发版仍只在 v* tag | byok-plan-v2 §7 verification_env | .github/workflows/build.yml |
...
- **TP5 已闭环**：CI 新增 `test` job（push/PR 跑 `vitest run`），构建/发版 `needs: test` 且仍只在 `v*` tag——"绿"不再依赖人手动关 sandbox 本地跑。⚠ 仍存：本地 sandbox 下需 `dangerouslyDisableSandbox` 才跑得起（CI runner 无此限制，不受影响）；byok-plan-v2 §7 verification_env 口径据此可定：**CI=权威 gate，本地=辅助**。
